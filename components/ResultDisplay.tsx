...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ data, characters, styleId, characterStyles, projectInputs, onReset, onBack }) => {
  ...
  const handleSingleImageGeneration = async (prompt: string, idx: number) => {
    try {
      ...
      const base64Image = await generateImage(prompt, referenceImages, styleId, characterStyles);
